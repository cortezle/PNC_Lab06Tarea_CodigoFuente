...
Matrix createLocalK(int element,mesh &m){
    float D,Ae;
    float Ec=m.getParameter(CONS_E);
    float Ac=m.getParameter(CONS_A);
    Matrix K,A,B,Bt,At;
    ...
    productRealMatrix(-Ec*Ac*Ae/(2*D*D),productMatrixMatrix(Bt,productMatrixMatrix(At,productMatrixMatrix(A,B,2,2,3),2,2,3),3,2,3),K);
...
Vector createLocalb(int element,mesh &m){
    ...
    float F = m.getParameter(CONS_F),J;
